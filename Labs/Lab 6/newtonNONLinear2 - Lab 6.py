...
def driver():
    ...
    elapsed = time.time()-t
    print(xstar)
    print('Slacker: the error message reads:',ier)
    print('Slacker: took this many seconds:',elapsed/20)
...
def forward_difference(f, s, h, var):

    if var == x:
        return (f(
   
    return (f(s+h, y) - f(s,y))/(h)
...
def SlackerNewton(x0, tol, Nmax):
    ...
    if count >= 1 and abs(iterates[count] - iterates[count - 1]) > [0.5,0.5]:
        J = evalJ(iterates[-1])
        Jinv = inv(J)
